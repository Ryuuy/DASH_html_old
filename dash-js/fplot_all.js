...
var steppingX = 60; // 15 second steps
...
fPlot.prototype.plot = function()
{
	...
	var maxY = (parseInt(maxBandwidth/1024/1024)+1)*1000-1, maxX = this.period;
	for(var n = 0; n < this.f.length; n++)
	{
		...
		for(var i=0;i<this.f[n].values.length;i++)
		{
            if(n!= 2)
            {
                ...
                if((this.f[n].timeStamps[i] - this.startTime)/1000 > maxX) maxX = (this.f[n].timeStamps[i] - this.startTime)/1000;		
            }
		}
	...
	this.canvas.beginPath();
	this.canvas.moveTo(0,0);
	this.canvas.lineTo(this.width - 50, 0);
	//this.canvas.moveTo(0,0);
	this.canvas.lineTo(this.width - 50,this.height - 100);
...
//	this.canvas.scale(1, -1);
	...
	for(var n=0; n <= maxX/steppingX;n++)
	{
		...
		this.canvas.fillText("Estimated Bandwidth", 10 + metrics.width/2, this.height - 75);
	...
	//	var metrics = this.canvas.measureText("Representation Bandwidth");
		...
		this.canvas.fillText("Representation Bandwidth", 10 + metrics.width*2, this.height - 75);
	...
	for(var n = 0; n < this.f.length; n++)
	{
        if(n==2)
        {
            ...
            var currentPlaybackTime = this.f[n].values[this.f[n].cnt-1];
            var barPosition = (((currentPlaybackTime)/maxX) * this.graphwidth);
	...
	//	this.canvas.strokeStyle = "rgba(0,0,0,1)";
		...
		for(var i=0;i<this.f[n].values.length;i++)
		{
			this.canvas.lineTo((2*i/maxX)*this.graphwidth, ((this.f[n].values[i]/(1024))/maxY)*this.graphheight);
		    this.canvas.lineTo((2*(i+1)/maxX)*this.graphwidth, ((this.f[n].values[i]/(1024))/maxY)*this.graphheight);
		}
		this.canvas.stroke();
		this.canvas.closePath();
