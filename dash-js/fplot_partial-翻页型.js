...
var steppingX = 5; // second steps in one frame range
...
fPlot.prototype.plot = function()
{
	...
	var steppingY = 1000; // 200 kbit steps for the Y-axis
	var maxX = 60; //画布中只容纳 maxX秒的图
...
//	this.canvas.scale(1, -1);
	...
	var currentRangeStart = parseInt(currentPlaybackTime/maxX)*maxX;
	var currentSegmentStart = parseInt(currentRangeStart/2);
	...
	for(var n=0; n <= maxX/steppingX;n++)
	{
		...
		this.canvas.font         = '10px sans-serif';
		this.canvas.textBaseline = 'top';
		var metrics = this.canvas.measureText(n*steppingX + currentRangeStart); 
	
		this.canvas.fillText(parseInt(n*steppingX + currentRangeStart), 50 + (((n*steppingX)/maxX) * this.graphwidth) - metrics.width/2, this.height - 95);
	}
        this.canvas.fillStyle    = '#00f';
        this.canvas.font         = '14px sans-serif';
        this.canvas.textBaseline = 'top';
        var metrics = this.canvas.measureText("t");
        this.canvas.fillText("t", this.graphwidth - metrics.width, this.height - 75);
		...
		this.canvas.fillText("Estimated Bandwidth" + unit, 10 + metrics.width/2, this.height - 75);
	...
	//	var metrics = this.canvas.measureText("Representation Bandwidth");
		...
		this.canvas.fillText("Representation Bandwidth" + unit, 10 + metrics.width*2, this.height - 75);
	...
	for(var n = 0; n < this.f.length; n++)
	{
        ...
        if (n == 0 || n == 1){
	     ...
	     if(n==3){
	     	var startTimeInDownload = this.f[n].timeStamps[0];
	     	this.canvas.strokeStyle = "rgba(7,195,5,1)";
            this.canvas.setLineDash([]);

            for(var i=0;i<this.f[n].values.length;i++)
	        {
	        	var startPoint = (this.f[n].timeStamps[i] - startTimeInDownload)/1000;
	        	if (startPoint>=currentRangeStart&& startPoint<=(currentRangeStart + maxX)){

	        		var endPoint = (this.f[n].timeStampsOver[i] - startTimeInDownload)/1000;
	        		var HighPoint = this.f[n].values[i];

	        		this.canvas.beginPath();
                    this.canvas.moveTo(((startPoint-currentRangeStart)/maxX)*this.graphwidth,0);
                    this.canvas.lineTo(((startPoint-currentRangeStart)/maxX)*this.graphwidth,(HighPoint/maxY/1024)*this.graphheight);
                    this.canvas.lineTo(((endPoint-currentRangeStart)/maxX)*this.graphwidth,(HighPoint/maxY/1024)*this.graphheight);
                    this.canvas.lineTo(((endPoint-currentRangeStart)/maxX)*this.graphwidth,0);
                    this.canvas.stroke();
	                this.canvas.closePath();
